...
# ═══════════════════════════════════════════════════════════════
# AgentQ AI Trading System - Docker Quick Start Script
...
check_encryption() {
    ...
    # 检查并生成 HYPERLIQUID_ENCRYPTION_KEY
    if ! is_env_configured "HYPERLIQUID_ENCRYPTION_KEY"; then
        print_warning "HYPERLIQUID_ENCRYPTION_KEY 未配置，正在生成..."
        local encryption_key=$(openssl rand -base64 32)
        set_env_var "HYPERLIQUID_ENCRYPTION_KEY" "$encryption_key"
        print_success "HYPERLIQUID_ENCRYPTION_KEY 已生成"
        generated=true
    fi
    ...
    print_info "  • HYPERLIQUID_ENCRYPTION_KEY: OK"
...
# ------------------------------------------------------------------------
start() {
    print_info "正在启动 AgentQ AI Trading System..."
    ...
    print_success "服务已启动！"
    print_info "API 端点：http://localhost:8802"
    print_info ""
...
status() {
    ...
    echo ""
    print_info "健康检查:"
    curl -s "http://localhost:8802/api/health" | jq '.' || echo "后端未响应"
}
...
# ------------------------------------------------------------------------
show_help() {
    echo "AgentQ AI Trading System - Docker 管理脚本"
    echo ""
    ...
    echo "  stop               停止服务"
    echo "  restart            重启服务"
    echo "  logs [service] [--errors] 查看日志（可选：指定服务名 app/postgres，--errors 仅看错误）"
    echo "  status             查看服务状态"
    echo "  build              构建镜像"
